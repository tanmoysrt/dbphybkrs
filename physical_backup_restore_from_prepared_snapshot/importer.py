...
class DatabaseImporter:
    ...
    def process(self):
        ...
        print("-- Starting the restoration process --")
        validate_and_warmup_start_time = time.time()
        self._warmup_myisam_files()
        print("Warmed up MyISAM table files")
        self._check_and_fix_myisam_table_files()
        print("Checked and fixed MyISAM table files")
        self._warmup_innodb_files()
        print("Warmed up InnoDB table files")
        ...
        print(
            "Validation and Warmup Time taken: {} seconds".format(
                round(restoration_start_time - validate_and_warmup_start_time, 2)
            )
        )
        print(
            "Restoration Time taken: {} seconds".format(
                round(restoration_end_time - restoration_start_time, 2)
            )
        )
