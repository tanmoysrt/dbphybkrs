...
class DatabaseExporter:
    def __init__(self, database:str, db_user:str, db_password:str, target_backup_file_path:str, db_host:str="localhost", db_port:int=3306, db_base_path:str="/var/lib/mysql"):
    ...
    def _perform_database_operations(self):
        ...
        flush_table_export_query = "FLUSH TABLES {} FOR EXPORT;".format(", ".join(self.tables))
    ...
    def _perform_file_operations(self):
        ...
        for table in self.tables:
            ...
            if table_file not in db_files:
                raise DatabsaeExportFileNotFoundError("IBD file for table {} not found".format(table))
            table_cfg_file = table + ".cfg"
            if table_cfg_file not in db_files:
                raise DatabsaeExportFileNotFoundError("CFG file for table {} not found".format(table))
        ...
        with tarfile.open(self.target_backup_file_path, "w:gz", compresslevel=1) as tar_file:
            # add the schema file
            ...
            for table in self.tables:
                ...
                tar_file.add(os.path.join(self.db_directory, table_ibd_file), arcname=table_ibd_file)
                table_cfg_file = table + ".cfg"
                tar_file.add(os.path.join(self.db_directory, table_cfg_file), arcname=table_cfg_file)
